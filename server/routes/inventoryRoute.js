...
router.post("/add", authMiddleware, async(req,res) => {
    try {
        ...
        if(req.body.inventoryType === "out"){
            ...
            // check is inventory is available
...
router.post("/filter", authMiddleware, async(req,res) => {
    try {
        const inventory = await Inventory.find(req.body.filters).limit(req.body.limit || 10).sort({createdAt: -1}).populate("donor").populate("hospital").populate("organization");
        return res.send({ success: true, data: inventory });
    } catch (error) {
        res.send({ success: false, message: error.message });
    }
